...
// d9ce3f09-3545-4ada-84f0-edc86dfdcd0f
static uint8_t service_uuid[16] = {
	/* LSB <---------------------------------------------> MSB */
	//first uuid, 16bit, [12],[13] is the value
	0x0f, 0xcd, 0xfd, 0x6d, 0xc8, 0xed, 0xf0, 0x84,
	0xda, 0x4a, 0x45, 0x35, 0x09, 0x3f, 0xce, 0xd9
};
...
static esp_ble_adv_data_t adv_data = {
	...
	.min_interval = 0x0006, //slave connection min interval, * 1.25 msec
	.max_interval = 0x0010, //slave connection max interval, * 1.25 msec
	.appearance = 0x00,
...
uint8_t char_value[] = {3, 0, 10, 0, 50, 0};
uint8_t char_save = 0;
bool char_value_change;
...
{
	uint16_t *s = (uint16_t*)char_value;
	ESP_LOGI(GATTS_TABLE_TAG,
		"param0 %" PRIu16 " ,param1 %" PRIu16 " , param2 %" PRIu16 "\n",
	 	s[0], s[1], s[2]);
}
...
static const esp_gatts_attr_db_t gatt_db[] =
{
	...
	{{ESP_GATT_AUTO_RSP},
		...
		ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
		GATTS_DEMO_CHAR_VAL_LEN_MAX, 1,
		(uint8_t*)&char_save}
	}
};

uint16_t handle_table[NUM_ARRAY_ELEMS(gatt_db)];

void dump_handles() {
	for (int i = 0; i < NUM_ARRAY_ELEMS(handle_table); i++)
		printf("handle[%d] = %"PRIu16"\n", i, handle_table[i]);
}
	...
	esp_ble_gap_cb_param_t *param)
{
	switch (event) {
		...
		case ESP_GAP_BLE_UPDATE_CONN_PARAMS_EVT:
			ESP_LOGI(GATTS_TABLE_TAG, "update connection params "
	...
	switch (event) {
		case ESP_GATTS_REG_EVT:
			...
			ret = esp_ble_gatts_create_attr_tab(gatt_db, gatts_if,
				NUM_ARRAY_ELEMS(gatt_db), SVC_INST_ID);
			if (ret){
				ESP_LOGE(GATTS_TABLE_TAG,
					"create attr table failed, error code = %x", ret);
			}
		break;
	   ...
	   break;
			...
			if (!param->write.is_prep){
				...
				if (param->write.handle == handle_table[2]) {
					memcpy(char_value, param->write.value, param->write.len <
						sizeof(char_value) ? param->write.len : sizeof(char_value));
					char_value_change = 1;
				} else {
					nvm_write("vbgl_save", char_value, sizeof(char_value));
				}
				/* send response when param->write.need_rsp is true*/
				if (param->write.need_rsp){
		...
		case ESP_GATTS_CREAT_ATTR_TAB_EVT:
			...
			else if (param->add_attr_tab.num_handle != NUM_ARRAY_ELEMS(gatt_db)){
				ESP_LOGE(GATTS_TABLE_TAG, "create attribute table abnormally, "
					"num_handle (%d) doesn't equal to HRS_IDX_NB(%d)",
					param->add_attr_tab.num_handle, NUM_ARRAY_ELEMS(gatt_db));
			} else {
				ESP_LOGI(GATTS_TABLE_TAG, "create attribute table successfully, "
					"the number handle = %d",param->add_attr_tab.num_handle);
...
void ble_init(void)
{
	...
	{
		uint8_t ds[sizeof(char_value)];
		if (nvm_read("vbgl_save", ds, sizeof(char_value))) {
			memcpy(char_value, ds, sizeof(char_value));
			ESP_LOGI(GATTS_TABLE_TAG, "Found saved values\n");
		}
	}
...

	{
		...
		if (mac != NULL) {
			...
			sprintf(dev_name + 4, "%02x%02x%02x", *(mac + 3), *(mac + 4),
			*(mac + 5));
		}
	}
	...
	if (ret){
		ESP_LOGE(GATTS_TABLE_TAG,
			"gatts app register error, error code = %x", ret);
		return;
	}
	...
	if (local_mtu_ret){
		ESP_LOGE(GATTS_TABLE_TAG,
			"set local  MTU failed, error code = %x", local_mtu_ret);
	}
